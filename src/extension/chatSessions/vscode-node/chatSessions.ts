...
const CLOSE_SESSION_PR_CMD = 'reea.copilot.cloud.sessions.proxy.closeChatSessionPullRequest';
export class ChatSessionsContrib extends Disposable implements IExtensionContribution {
	...
	private registerCopilotCloudAgent() {
		...
		this.copilotCloudRegistrations.add(
			vscode.commands.registerCommand('reea.copilot.cloud.resetWorkspaceConfirmations', () => {
				cloudSessionsProvider.resetWorkspaceContext();
			})
		);
		this.copilotCloudRegistrations.add(
			vscode.commands.registerCommand('reea.copilot.cloud.sessions.openInBrowser', async (chatSessionItem: vscode.ChatSessionItem) => {
		...
		this.copilotCloudRegistrations.add(
			vscode.commands.registerCommand('reea.copilot.cloud.sessions.installPRExtension', async () => {
