...
export class SearchPanelCommands extends Disposable {
	constructor(
		...
		super();
		this._register(vscode.commands.registerCommand('reea.copilot.search.markHelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Helpful);
		}));
		this._register(vscode.commands.registerCommand('reea.copilot.search.markUnhelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Unhelpful);
		}));
		this._register(vscode.commands.registerCommand('reea.copilot.search.feedback', () => {
			this.sendFeedback(SearchFeedbackKind.Feedback);
			vscode.commands.executeCommand('reea.copilot.report', `Copilot search feedback: "${SemanticSearchTextSearchProvider.latestQuery}"`);
		}));
	}
