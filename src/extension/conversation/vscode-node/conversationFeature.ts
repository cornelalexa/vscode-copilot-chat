...
export class ConversationFeature implements IExtensionContribution {
	...
	set enabled(value: boolean) {
		...
		vscode.commands.executeCommand('setContext', 'reea.copilot.interactiveSession.disabled', !value);
	}
	...
	private registerCommands(options: IConversationOptions): IDisposable {
		...
		[
			vscode.commands.registerCommand('reea.copilot.interactiveSession.feedback', async () => {
				return vscode.env.openExternal(vscode.Uri.parse(FEEDBACK_URL));
			}),
			vscode.commands.registerCommand('reea.copilot.chat.compact', () => vscode.commands.executeCommand('workbench.action.chat.open', { query: '/compact' })),
			vscode.commands.registerCommand('reea.copilot.terminal.explainTerminalLastCommand', async () => this.triggerTerminalChat({ query: `/${TerminalExplainIntent.intentName} #terminalLastCommand` })),
			vscode.commands.registerCommand('reea.copilot.terminal.fixTerminalLastCommand', async () => generateTerminalFixes(this.instantiationService)),
			vscode.commands.registerCommand('reea.copilot.terminal.generateCommitMessage', async () => {
			...
			vscode.commands.registerCommand('reea.copilot.git.generateCommitMessage', async (rootUri: vscode.Uri | undefined, _: vscode.SourceControlInputBoxValueProviderContext[], cancellationToken: vscode.CancellationToken | undefined) => {
			...
			vscode.commands.registerCommand('reea.copilot.git.resolveMergeConflicts', async (...resourceStates: (vscode.Uri | vscode.SourceControlResourceState)[]) => {
			...
			vscode.commands.registerCommand('reea.copilot.devcontainer.generateDevContainerConfig', async (args: DevContainerConfigGeneratorArguments, cancellationToken?: vscode.CancellationToken) => {
			...
			vscode.commands.registerCommand('reea.copilot.chat.openUserPreferences', async () => {
	...
	private registerTerminalQuickFixProviders() {
		...
		return combinedDisposable(
			vscode.window.registerTerminalQuickFixProvider('reea-copilot-chat.fixWithCopilot', {
				provideTerminalQuickFixes(commandMatchResult, token) {
					...
					return [
						{
							command: 'reea.copilot.terminal.fixTerminalLastCommand',
							title: vscode.l10n.t('Fix using Copilot')
						},
						{
							command: 'reea.copilot.terminal.explainTerminalLastCommand',
							title: vscode.l10n.t('Explain using Copilot')
						}
					];
				}
			}),
			vscode.window.registerTerminalQuickFixProvider('reea-copilot-chat.generateCommitMessage', {
				provideTerminalQuickFixes: (commandMatchResult, token) => {
					return this.enabled ? [{
						command: 'reea.copilot.terminal.generateCommitMessage',
...
function registerSearchIntentCommand(): IDisposable {
	return vscode.commands.registerCommand('reea.copilot.executeSearch', async (arg: FindInFilesArgs) => {
