...
const showContextInspectorViewContextKey = `reea.copilot.chat.showContextInspectorView`;
export class InlineCompletionContribution implements vscode.Disposable, TokenBudgetProvider {
	...
	constructor(
		...
		if (languageContextService instanceof LanguageContextServiceImpl) {
			this.disposables.add(vscode.commands.registerCommand('reea.copilot.debug.showContextInspectorView', async () => {
				await vscode.commands.executeCommand('setContext', showContextInspectorViewContextKey, true);
				await vscode.commands.executeCommand('reea-context-inspector.focus');
			}));
			this.disposables.add(vscode.window.registerTreeDataProvider('reea-context-inspector', new InspectorDataProvider(languageContextService)));
		}
